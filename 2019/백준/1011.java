 ...
 * 
 * 좀 어려웠다. 구글링으로 노가다+규칙 참고하여 겨우 풀었다.
 * 참고:
...
public class Main {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<T; i++) {
			testCase[i][0]=input.nextInt();
			testCase[i][1]=input.nextInt();
		}
		
		for(int i=0; i<T; i++) {
			int value = testCase[i][1]-testCase[i][0];//거리
			if(value==0) System.out.println(0);//0 제외
			else System.out.println(횟수(value));
		}
		
	}
	
	public static int 횟수(int value) {
		long N = 1;//N을 long으로 선언해야지 N제곱이 int 범위 벗어날때 2^31을 처리할수 있었다.. 개짜증
		int num = 1;
		while(true) {
			if(value<=N*N) break;
			else if(value<=N*N+N) {
