...
public class Main {
	private static String[] 명령어 = new String[] {
	...
	public static void main(String[] args) {
		...
		int N = input.nextInt();
		input.nextLine();
		String[] 명령어들 = new String[N];
		
		for(int i=0; i<N; i++) 명령어들[i]=input.nextLine();
		
		ArrayList<Integer> stack = new ArrayList<Integer>();
		for(String value:명령어들) {
			String temp = value.substring(0, 2);
			int i= 0;
			for(;i<명령어.length; i++) {
				if(temp.equals(명령어[i].substring(0, 2))) break;
			}
